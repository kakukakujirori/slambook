...
int main(int argc, char **argv) {
    ...
    // let's randomly pick pixels in the first image
    // and generate some 3d points in the first image's frame
    cv::RNG rng;
    int nPoints = 2000;
    int boarder = 20;
    ...
    for (int i = 0; i < nPoints; i++) {
        int x = rng.uniform(boarder, left_img.cols - boarder);  // don't pixk pixels near boarder
        int y = rng.uniform(boarder, left_img.rows - boarder);  // don't pixk pixels near boarder
...
void DirectPoseEstimationSingleLayer(
    ...
    cout << "direct method for single layer: " << time_used.count() << " seconds." << endl;

    // plot the projection pixels here
    cv::Mat img2_show;
