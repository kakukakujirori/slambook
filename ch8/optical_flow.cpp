...
public:
    OpticalFlowTracker(
        ...
        success(success_), has_initial(has_initial_) {}
 ...
 * @param [in] kp1 keypoints in img1
 * @param [out] kp2 keypoints in img2, if empty, use initial guess in kp1
 * @param [out] success true if a keypoint is tracked successfully
...
/**
 * @brief get a gray scale value from reference image (bilinear interpolated)
 * @param [in] img
 * @param [in] x
 * @param [in] y
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++) {
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    if (!inverse) {
                        ...
                        // NOTE this J does not changewhen dx, dy is updated, so we can store it and only compute error
                        J = -1.0 * Eigen::Vector2d(
