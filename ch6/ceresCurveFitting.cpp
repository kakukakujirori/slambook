...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < N; i++) {
        problem.AddResidualBlock(  // add i-th residual into the problem
            // use auto-diff, template params:
            // residual type, output dimension, input dimension should be
            // the same as the struct written before
    ...
    ceres::Solver::Options options;  // actually there're lots of params that can be adjusted
