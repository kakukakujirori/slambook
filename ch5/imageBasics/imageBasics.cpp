...
int main(int argc, char **argv) {
    ...
    for (size_t y = 0; y < image.rows; y++) {
        ...
        for (size_t x = 0; x < image.cols; x++) {
            ...
            for (int c = 0; c < image.channels(); c++) {
                unsigned char data = data_ptr[c];  // dagta should be pixel of I(x,y) in c-th channel
            }
        }
    }
    ...
    image_another(cv::Rect(0, 0, 100, 100)).setTo(0);  // set top-left 100x100 block to zero
