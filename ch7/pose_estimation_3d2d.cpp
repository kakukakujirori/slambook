...
int main(int argc, char **argv) {
    ...
    for (DMatch m: matches) {
        ushort d = d1.ptr<unsigned short>(static_cast<int>(keypoints_1[m.queryIdx].pt.y))[static_cast<int>(keypoints_1[m.queryIdx].pt.x)];
    ...
    Mat r, t;
    solvePnP(pts_3d, pts_2d, K, Mat(), r, t, false);  // Call OpenCV's PnP solver, optionally EPNP, DLS, etc.
    Mat R;
    cv::Rodrigues(r, R);  // r is in the form of a rotation vector, which is converted to a matrix using the Rodrigues formula
