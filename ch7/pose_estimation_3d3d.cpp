...
int main(int argc, char **argv) {
    ...
    for (DMatch m: matches) {
        ushort d1 = depth1.ptr<unsigned short>(static_cast<int>(keypoints_1[m.queryIdx].pt.y))[static_cast<int>(keypoints_1[m.queryIdx].pt.x)];
        ushort d2 = depth2.ptr<unsigned short>(static_cast<int>(keypoints_2[m.trainIdx].pt.y))[static_cast<int>(keypoints_2[m.trainIdx].pt.x)];
...
void pose_estimation_3d3d(const vector<Point3f> &pts1,
    ...
    Eigen::Matrix3d R_ = U * V.transpose();  // ここ間違ってる気がするんだけどなあ
    if (R_.determinant() < 0) {
        R_ *= -1;
    }
...
void bundleAdjustment(
    ...
    const vector<Point3f> &pts2,
    Mat &R, Mat &t) {
    // Build graph optimization, let's define g2o first
    ...
    // vertex
    VertexPose *pose = new VertexPose();  // camera vertex_pose
    pose->setId(0);
